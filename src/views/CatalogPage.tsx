...
export function CatalogPage({ lang, navigate }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--background)' }}>
      {/* Page header */}
      <div style={{ backgroundColor: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '32px 24px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'var(--foreground)', margin: '0 0 6px' }}>
            {t('Catalogue des formations', 'كتالوج الدورات', lang)}
          </h1>
          <p style={{ fontSize: 15, color: 'var(--muted-foreground)', margin: '0 0 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: '1 1 300px' }}>
              ...
              <input
                ...
                style={{
                  ...
                  color: 'var(--foreground)', outline: 'none', backgroundColor: 'var(--surface)',
            ...
            <select
              ...
              style={{
                height: 44, border: '1.5px solid var(--border)', borderRadius: 10,
                padding: '0 36px 0 12px', fontSize: 14, color: 'var(--foreground)',
                backgroundColor: 'var(--surface)', cursor: 'pointer', outline: 'none',
                appearance: 'none',
            ...
            <button
              ...
              style={{
                ...
                backgroundColor: activeFilterCount > 0 ? 'var(--accent)' : 'var(--surface)',
                color: activeFilterCount > 0 ? 'var(--accent-foreground)' : 'var(--foreground)',
      ...
      {/* Filters panel */}
      {showFilters && (
        <div style={{ backgroundColor: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '24px', animation: 'fadeIn 0.2s ease' }}>
                ...
                {platforms.filter(p => p.status === 'available').map(p => (
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: selectedPlatform === p.name ? 'var(--accent)' : 'var(--surface-secondary)',
                      color: selectedPlatform === p.name ? 'var(--accent-foreground)' : 'var(--foreground)',
                      border: 'none',
                    }}
                  >
                    {p.name}
                ...
                {categories.slice(0, 5).map(c => (
                  <button
                    ...
                    style={{
                      padding: '5px 12px', borderRadius: 8, fontSize: 13, fontWeight: 500,
                      cursor: 'pointer', transition: 'all 0.15s',
                      backgroundColor: selectedCategory === c.id ? 'var(--accent)' : 'var(--surface-secondary)',
                      color: selectedCategory === c.id ? 'var(--accent-foreground)' : 'var(--foreground)',
                      border: 'none',
                    }}
                  >
                ...
                {levels.map(lv => (
                  <button
                    ...
                    style={{
                      padding: '5px 12px', borderRadius: 8, fontSize: 13, fontWeight: 500,
                      cursor: 'pointer', transition: 'all 0.15s',
                      backgroundColor: selectedLevel === lv.id ? 'var(--accent)' : 'var(--surface-secondary)',
                      color: selectedLevel === lv.id ? 'var(--accent-foreground)' : 'var(--foreground)',
                      border: 'none',
                    }}
                  >
                    {lang === 'ar' ? lv.ar : lv.fr}
